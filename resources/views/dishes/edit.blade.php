                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('dishes.update', $dish) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input id="name" type="text" name="name" value="{{ old('name', $dish->name) }}" required autofocus
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="3" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{{ old('description', $dish->description) }}</textarea>
                            @error('description')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input id="price" type="number" name="price" value="{{ old('price', $dish->price) }}" required step="0.01" min="0"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                            @error('price')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                                <option value="Voorgerecht" {{ old('type', $dish->type) == 'Voorgerecht' ? 'selected' : '' }}>Voorgerecht</option>
                                <option value="Hoofdgerecht" {{ old('type', $dish->type) == 'Hoofdgerecht' ? 'selected' : '' }}>Hoofdgerecht</option>
                                <option value="Nagerecht" {{ old('type', $dish->type) == 'Nagerecht' ? 'selected' : '' }}>Nagerecht</option>
                            </select>
                            @error('type')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                            ...
                            <select multiple id="articles" name="article_ids[]"
                                ...
                                @foreach($articles as $article)
                                    <option value="{{ $article->id }}"
                                        {{ in_array($article->id, old('article_ids', $selectedArticleIds)) ? 'selected' : '' }}>
                                        {{ $article->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                                {{ __('Update Dish') }}
                            </button>
                        </div>
                    </form>
